...
public:
    ...
    HASH_INDEX_T next() 
    {
        //if(this->numProbes_ == this_->m_)
        //if(this->numProbes == this->m_)
        if(this->numProbes_ == this->m_)
        {
            //return this_->npos;
            return this->npos;
        }
        //HASH_INDEX_T loc = (this->start_ + this_ -> numProbes_ * dhstep_) % this->m_;
...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
       :  hash_(hash), kequal_(kequal), prober_(prober), totalProbes_(0), mIndex_(0), resizeAlpha_(resizeAlpha), numItems_(0)
{
...
{
    double load = static_cast<double>(numItems_) / CAPACITIES[mIndex_];
    //if(load >= resizeAlpha){
    ...
    if(location == npos)
    {
      throw std::logic_error("No available location for insert");
    }
    if(table_[location] == nullptr || table_[location]->deleted)
    {
...
void HashTable<K,V,Prober,Hash,KEqual>::remove(const KeyType& key)
{
  ...
  if(location != npos && table_[location] != nullptr && !table_[location]->deleted)
  {
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
  ...
  {
    throw std::logic_error("No more capacity sizes available");
  }
